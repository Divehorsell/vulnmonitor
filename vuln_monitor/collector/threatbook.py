...
class ThreatBookCollector(BaseCollector):
    ...
    base_url = "https://x.threatbook.com/api/v1"

    def collect(self) -> list[dict]:
        results = []
        try:
            data = self.fetch_json(f"{self.base_url}/vulnerabilities", params={"page": 1, "size": 30})
            vulns = data.get("data", data.get("list", []))
            for item in vulns[:30]:
                cve_id = item.get("cve_id", item.get("cveId", ""))
                raw = {
                    "cve_id": cve_id or f"TB-{item.get('id', '')}",
            ...
            logger.error(f"[ThreatBook] Collection failed: {e}")

        logger.info(f"[ThreatBook] Collected {len(results)} advisories")
        return results
