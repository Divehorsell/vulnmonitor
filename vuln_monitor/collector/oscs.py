...
class OSCSCollector(BaseCollector):
    ...
    base_url = "https://www.oscs1024.com/api/v1"

    def collect(self) -> list[dict]:
        results = []
        try:
            data = self.fetch_json(f"{self.base_url}/vulnerability/list", params={"page": 1, "per_page": 30})
            vulns = data.get("data", data.get("list", []))
            for item in vulns[:30]:
                cve_id = item.get("cve_id", item.get("cveId", ""))
                raw = {
                    "cve_id": cve_id or f"OSCS-{item.get('id', '')}",
            ...
            logger.error(f"[OSCS] Collection failed: {e}")

        logger.info(f"[OSCS] Collected {len(results)} advisories")
        return results
